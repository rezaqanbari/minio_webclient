...
6. **سیستم احراز هویت و امنیت (Authentication & Security):**
   - صفحه لاگین مدرن و محافظت کامل از تمام بخش‌های پنل و فایل‌ها
   - احراز هویت مبتنی بر کوکی امن و رمزنگاری‌شده با انقضای خودکار
   - قابلیت تنظیم نام کاربری و رمز عبور دلخواه از طریق فایل `.env` یا `appsettings.json` (پیش‌فرض: `admin` / `Admin@123456`)
   - دکمه خروج امن (Logout) در نوار بالای سایت
